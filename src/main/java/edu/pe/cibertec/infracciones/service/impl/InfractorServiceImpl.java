...
public class InfractorServiceImpl implements IInfractorService {
    ...
    @Override
    public Double calcuDeuda(Long infractorId){
        List<Multa> pendientes = multaRepository.findByInfractor_IdAndEstado(infractorId, EstadoMulta.PENDIENTE);
        List<Multa> vencida = multaRepository.findByInfractor_IdAndEstado(infractorId, EstadoMulta.VENCIDA);
        double totalPendientes = pendientes.stream().mapToDouble(Multa::getMonto).sum();
        double totalVencidas = vencida.stream().mapToDouble(m -> m.getMonto() * 1.15).sum();
        return totalPendientes + totalVencidas;
    }

    @Override
    public void desasignarVehiculo(Long infractorId, Long vehiculoId){
        Infractor infractor = infractorRepository.findById(infractorId).orElseThrow(
                () -> new InfractorNotFoundException(infractorId)
        );

        boolean tienePendientes = multaRepository
                .existsByVehiculo_IdAndInfractor_IdAndEstado(vehiculoId, infractorId, EstadoMulta.PENDIENTE);

        if (tienePendientes){
            throw new RuntimeException(
                    "Este vehiculo no se puede desasignar"
            );
        }
